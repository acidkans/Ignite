...
### architektura / API
- `ui-stala` `PURCHASE_STATUS_META` — dołożony kod `PARTIALLY_DELIVERED` („Dostawa częściowa") między `ORDERED` a `DELIVERED`. Opisuje towar W DRODZE: część dojechała, reszta nie. Kod rodzi się razem z tym etapem, więc w bazie nie ma ani jednego wiersza do migracji.
- `ui-przycisk` `StatusHint` — PODPOWIEDŹ statusu realizacji pod dropdownem w zakładce Realizacja, liczona z dziennika wpisów (`realizationOf`): 0 < Σ ilości < plan → zakup „Dostawa częściowa", Σ ilości >= plan → zakup „Dostarczone", pierwszy wpis → wykonanie „W toku". Klik zapisuje tą samą drogą co ręczna zmiana; brak kliku nie zmienia niczego. Świadomie NIE jest automatem — wpis bywa zaliczką, a nie dostawą, i tylko człowiek wie, który to przypadek.
- `ui-funkcja` `suggestAxisStatus` — reguła podpowiedzi w jednym miejscu, wspólna dla komórki i testu. Podpowiedź nigdy nie COFA (porównanie przez `AXIS_STATUS_ORDER`): pozycja „Zafakturowana" nie wraca na „Dostarczone", a wstrzymana nie odwiesza się sama. Pozycja z ręcznym znacznikiem „rozliczone" nie dostaje podpowiedzi w ogóle.
- `ui-funkcja` `aggregateBranchStatus` / `buildAggregatedStatusMap` — agregacja gałęzi uogólniona na TRZY osie parametrem `axis` (`plan` | `purchase` | `exec`). Dotychczasowa reguła bez zmian: rekurencja zatrzymuje się na pierwszym węźle z własnym statusem, a pozycja kosztowa z dziećmi liczy się jako liść. Liść bez danej osi (praca nie ma zakupu, paliwo nie ma wykonania) WYPADA z sumy zamiast wchodzić do niej jako wartość startowa.
...
- `ui-kolumna` `realization-export-status-cols` — arkusz „Realizacja" w eksporcie Excel dostał trzy kolumny statusu (P „Status oferty", Q „Status zakupu", R „Status wykonania"), w tej samej kolejności co na ekranie: po kwotach, przed „Rozliczone". Oś, której dany typ liścia NIE MA, dostaje „—", a nie pustą komórkę — puste znaczyłoby „nikt jeszcze nie ustawił". Oś wykonania niesie stan WYLICZONY, więc pozycja odebrana protokołem wychodzi w Excelu jako „Odebrane", tak samo jak w tabeli.
- Wstawienie tych kolumn przesunęło „Rozliczone" P→S i „Wpisy" Q→T; razem z nimi przesunięto sumę wpisów w wierszu „Razem" i `COUNTIFS` w arkuszu „Podsumowanie", który liczy rozliczone pozycje. Zakres autofiltra `A1:R1` → `A1:U1`. Arkusz „Zakupy" bez zmian — jego wiersz to POJEDYNCZY wpis, a status jest cechą pozycji i powtarzałby się w każdej dostawie.

### słownik
- dodano `AXIS_STATUS_META`, `AXIS_STATUS_ORDER`, `axisStatusCodeOf`, `suggestAxisStatus`, `handedOverFromProtocol` — `wbsConstants.js`
- dodano `execCodeOf`, `StatusHint`, `BranchAxisBadge`, `RealizationGroupRow`, `hints`, `groups`, `collapsedGroups`, `nodeById`, `odbiorByRoot`, `fetchOdbior` — `RealizationTab.jsx`

### testy
- `test/status-etap5.test.mjs` — podpowiedzi (w tym „nie cofa" i „pozycja rozliczona nie dostaje"), wyliczone „Odebrane" i zgodność agregacji drzewiastej z płaską na każdej z trzech osi. Uruchamiać z `apps/frontend`: `node ../../test/status-etap5.test.mjs`.
- `test/sprawdz-eksport-statusy.py` — kontrola WYGENEROWANEGO pliku .xlsx: czy trzy kolumny statusu stoją na P/Q/R, czy formuły odwołujące się do kolumn za nimi przesunęły się razem z nimi i czy oś, której liść nie ma, ma „—". Uruchamiać na pobranym eksporcie: `python test/sprawdz-eksport-statusy.py <plik.xlsx>`.

### wytyczne
- `ui-funkcja` `suggestAxisStatus` — podpowiedź statusu ZAWSZE wymaga zatwierdzenia klikiem. Wpis realizacji jest dowodem, że coś się wydarzyło, ale nie mówi CO: zaliczka, dostawa częściowa i pełna dostawa wyglądają w dzienniku tak samo.
- `ui-funkcja` `handedOverFromProtocol` — `HANDED_OVER` nie jest zapisywany do `WbsNode.execStatus`. Zapisany kod zostałby na pozycji po wycofanym protokole, a stan odbioru ma żyć razem z dokumentem, który go tworzy.
- `ui-funkcja` `axisGateOf` — bramka blokuje WYŁĄCZNIE etap, który jeszcze nie ruszył (oś stoi na NULL). Oś z zapisaną wartością pokazujemy zawsze, nawet gdy warunek nie jest spełniony: na produkcji siedzą pozycje „Nowe" z zapisanym `ORDERED` po migracji i schowanie ich za plakietką ukryłoby dokładnie ten rozjazd, który trzeba naprawić. Bramka ma nie pozwolić ZACZĄĆ etapu poza kolejnością, a nie przepisywać historii.
- Nowy kod na osi realizacji trzeba dopisać do `AXIS_STATUS_ORDER`, inaczej podpowiedzi potraktują go jak stan startowy i będą potrafiły go cofnąć.
- Wstawienie kolumny w środek arkusza Excel przesuwa litery WSZYSTKICH kolumn za nią — razem z każdą formułą, która się do nich odwołuje, także z innego arkusza (`Realizacja!$S$`) — i z zakresem autofiltra. Po każdej takiej zmianie przepuść pobrany plik przez `test/sprawdz-eksport-statusy.py`: ExcelJS niczego tu nie sprawdza, a błędna formuła nie wywala eksportu, tylko po cichu liczy złą kolumnę.
- Nagłówek gałęzi w zakładce Realizacja sumuje pozycje WIDOCZNE po filtrach — ta sama zasada co stopka tabeli. Rozjazd między plakietką a wierszami pod nią czytałoby się jak błąd danych, a nie jak inny zakres.

...
### architektura / API
- `ui-funkcja` `handleExportGanttExcel` — eksport arkusza „Harmonogram" dostał po prawej siatkę timeline: kolumny grupowane wg aktualnego widoku Gantta (dzień/tydzień/miesiąc). Komórki kolorowane jak na wykresie — niebieski `#1d4ed8` = dzień roboczy zadania, szary = weekend/święto, zielony/czerwony nagłówek = marker startu/końca projektu. Pierwsze 4 kolumny (Zadanie/Od/Do/Dni) zamrożone (`xSplit`).
...
- `ui-funkcja` `handleExportGanttExcel` — kolumna D „Dni robocze" jest teraz formułą Excela liczoną z dat: `NETWORKDAYS(B,C-1,Dni_wolne!…)` (gałąź standardowa) lub `C-B` (gałąź pracująca w dni wolne). Święta trafiają do ukrytego arkusza `Dni_wolne`. Kolumna przelicza się po edycji dat w pliku.

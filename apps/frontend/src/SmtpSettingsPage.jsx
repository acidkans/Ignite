...
// Panel ADMIN — konfiguracja serwera poczty wychodzącej (SMTP). Hasło write-only:
// backend nie zwraca go nigdy (tylko flaga hasPassword); puste pole = bez zmiany hasła.
export default function SmtpSettingsPage() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/smtp`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
    ...
    <div className="p-6 overflow-auto custom-scrollbar h-full">
      <div className="animate-fade-in flex flex-col gap-6 w-full max-w-4xl mx-auto">
        <div className="glass-panel p-6 rounded-2xl border border-white/5 bg-gradient-to-br from-blue-600/10 to-purple-600/10 flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-white">Poczta wychodząca (SMTP)</h3>
            <p className="text-xs text-blue-300 opacity-60">Konfiguracja używana do wysyłki eksportów i powiadomień e-mail</p>
          </div>
        </div>

...
import { Save, CheckCircle, MapPin, Phone, Mail, User, Info, Navigation } from 'lucide-react';

export default function SiteInfoTab({ nodeId, nodeName }) {
    ...
    const fetchSite = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_URL}/sites/${nodeId}`, {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = {
                ...formData,
                addressLatitude: isNaN(lat) ? null : lat,
                addressLongitude: isNaN(lng) ? null : lng,
            };
            ...
            if (res.ok) {
                ...
                alert('Błąd zapisu danych lokalizacji');
            }
        } catch (err) {
        ...
        <div className="animate-fade-in flex flex-col gap-6 w-full max-w-4xl mx-auto">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-white/5 bg-gradient-to-br from-blue-600/10 to-purple-600/10 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-white">{nodeName || site?.processNode?.name || 'Nowa Lokalizacja'}</h3>
                        <p className="text-xs text-blue-300 opacity-60">Szczegóły i konfiguracja punktu geograficznego</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <section className="glass-panel p-6 rounded-2xl border border-white/5 bg-white/[0.02] flex flex-col gap-4">
                    ...
                    <div>
                        <label className={labelCls}>Numer Lokalizacji</label>
                        <input name="number" value={formData.number} onChange={handleChange} className={inputCls} placeholder="np. SITE-001" />
                    </div>
                    <div>
            ...
            <div className="flex justify-end pt-4 border-t border-white/5">
                ...
                >
                    ...
                    <span>{saving ? 'Zapisywanie...' : saved ? 'Zapisano pomyślnie!' : 'Zapisz dane lokalizacji'}</span>
                </button>
            </div>
        </div>
    );
